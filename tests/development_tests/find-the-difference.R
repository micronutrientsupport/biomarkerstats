...
SummaryStats <- function(theData,
  ...
  for (thresholdName in names(thresholds)) {
    ...
    lower <- thresholds[[thresholdName]]$lower
    upper <- thresholds[[thresholdName]]$upper
    
    # Default case for 'basic' thresholds
    if (is.null(lower)) {
      DataUse[[thresholdName]] <- ifelse(DataUse[, biomarkerField] <= as.numeric(upper), 1, 0)
    } else if (is.null(upper)) {
      DataUse[[thresholdName]] <- ifelse(DataUse[, biomarkerField] > as.numeric(lower), 1, 0)
    } else {
      DataUse[[thresholdName]] <- ifelse(DataUse[, biomarkerField] > as.numeric(lower) & DataUse[, biomarkerField] <= as.numeric(upper), 1, 0)
    }
  }
